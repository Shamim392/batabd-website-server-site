...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.agoku.mongodb.net/myFirstDatabase?retryWrites=true&w=majority`;
...
async function run(){
    try{
        ...
        const database = client.db("batabd-DB");
...
app.get('/',(req,res)=>{
    res.send('BATA_BD SERVER SITE RUNNING')
})

app.listen(port, ()=>{
  console.log('BATA_BD Server running on:',port);
})